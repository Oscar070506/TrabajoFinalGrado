...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormatTimePipe, TimeAgoPipe, TranslateModule],
...
export class UserAbout implements OnInit {
  ...
  fetchAll(): void {
    ...
    forkJoin({
      ...
      error: err => {
        this.error = this.translate.instant('PROFILE.ABOUT.ERROR', { msg: err.message });
        this.loading = false;
        this.cdr.detectChanges();
      }
    });
  ...
  private processRuns(runs: any[]): void {
    ...
    this.totalTimeSecs = runs.reduce(
      (acc, r) => acc + (r?.times?.primary_t ?? 0), 0
    );
